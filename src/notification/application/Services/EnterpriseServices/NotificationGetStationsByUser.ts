...
export class NotificationGetStationsByUserUseCase {
    constructor(
    ...
    async run(data: any) {
        const event = "enterprise.get-stations-by-user";

        if (!data) {
            throw new Error("No se pudo obtener la estación (datos insuficientes)");
        }
